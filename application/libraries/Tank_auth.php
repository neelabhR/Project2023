...
class Tank_auth
{
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$this->ci->session->set_userdata(array(
								...
								'admin'		=> ($user->role_id == 1 || $user->role_id == 3) ? array($user->id, $user->username, $user->role_id) : null
						));
	...
	function logout()
	{
		if(config_item('stop_timer_logout') == 'TRUE'){
			...
			if (!is_null($res = $this->ci->users->create_user($data, $profile, !$email_activation, $company))) {
				$data['user_id'] = $res['user_id'];
                                ...
                                if (intval($com->primary_contact) == 0) {
                                    $this->ci->db->where('co_id',$company_id)->update('companies',array('primary_contact'=>$res['user_id']));
                                }
	...
	* @return int
	*/
	function is_company_available($company,$email)
	{
	...
	function create_remote_login($user_id)
	{
		...
		if ($this->ci->remote_login->set($user_id, md5($key), $expiration)) { return md5($key); }
	 ...
	 * @return	void
	 */
	function remote_login($key = FALSE)
	{
		if (!$this->is_logged_in() AND !$this->is_logged_in(FALSE)) {			// not logged in (as any user)
			if ($key) {
                                $this->ci->load->model('tank_auth/remote_login');
                                if (!is_null($user = $this->ci->remote_login->get($key))) {
                                        if (strtotime($user->expires) < time()) { return FALSE; }
