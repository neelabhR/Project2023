 ...
 * Description: Namecheap Registrar Integration
 * Author: Hosting Billing
 * Author URI: www.hostingbilling.net
 */

class Namecheap extends Hosting_Billing
{          
    ...
    { 
		...
		public function __construct() {
			...
			$this->api_ip = (config_item('namecheap_ip') != '' ) ? config_item('namecheap_ip') : $this->detect_ip();
		}


		public function namecheap_config ($values = null)
		{
			$config = array(
				...
				array(
					'label' => lang('username'), 
					'id' => 'username',
					'value' => isset($values) ? $values['username'] : ''
				),

				array(
					'label' => lang('ip'), 
					'id' => 'ip',
					'value' => isset($values) ? $values['ip'] : ''
				)
	...
	public function check_balance ()
	{
		...
		return $this->Response->UserGetBalancesResult->attributes()->AvailableBalance ." ".$this->Response->UserGetBalancesResult->attributes()->Currency;
	...
	public function admin_options()
		{
			return '<a class="btn btn-primary btn-sm" href="'.base_url().'registrars/config/namecheap" data-toggle="ajaxModal" title="'.lang('settings').'"><i class="fa fa-pencil"></i> '.lang('settings').'</a>                               
	 				<a class="btn btn-warning btn-sm" href="'.base_url().'namecheap/import_domains/"  ><i class="fa fa-download"></i> '.lang('import_accounts').'</a>
					<a class="btn btn-success btn-sm" href="'.base_url().'registrars/check_balance/namecheap" data-toggle="ajaxModal" title="'.lang('check_balance').'" ><i class="fa fa-info-circle"></i> '.lang('check_balance').'</a>';
		}
