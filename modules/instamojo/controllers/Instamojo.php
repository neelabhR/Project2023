...
{   
	...
	function __construct()
	{
		parent::__construct();		
        User::logged_in();
        $this->config = get_settings('instamojo');
        if(!empty($this->config))
        {             
            $this->sandbox = $this->config['mode'] == 'test' ? true : false;
            $this->api_key = $this->config['api_key'];
            $this->oath_token = $this->config['oath_token'];
       }			
    }


    public function instamojo_config ($values = null)
    {
        $config = array(
            array(
                'label' => lang('mode'),
            ...
            array(
                'label' => lang('instamojo_api_key'), 
                'id' => 'api_key',
                'value' => isset($values) ? $values['api_key'] : ''
            ), 
            array(
                'label' => lang('instamojo_oath_token'), 
                'id' => 'oath_token',
                'value' => isset($values) ? $values['oath_token'] : ''
            ) 
        ); 
    ...
    {       $protocol = isset($_SERVER['HTTPS']) && strcasecmp('off', $_SERVER['HTTPS']) !== 0 ? "https" : "http";
            ...
            if($this->sandbox == true) {            
                 $api = new Instamojo\Instamojo($this->api_key, $this->oath_token, 'https://test.instamojo.com/api/1.1/');
            }
            else {
                $api = new Instamojo\Instamojo($this->api_key, $this->oath_token);
            }
    ...
    function verify($invoice = null) {
        ...
        if($verified){
            ...
            if($status == "Credit" || $status == "Completed"){ 
                ...
                $paid = Applib::convert_currency($response['payment']['currency'], $amount);
