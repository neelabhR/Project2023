...
class AppLib {
    ...
    static function get_table_field($table, $table_field, $where_criteria = array()) {
    ...
    function init()
    {  
         $k = self::$db->where('config_key','purchase_code')->get(self::$config_table)->row();
        if (!preg_match("/^(\w{8})-((\w{4})-){3}(\w{12})$/",$k->value))
        {Applib::update(Applib::$config_table,
            array('config_key'=>'demo_mode'),array( 'value'=>'TRUE'));Applib::switchoff();} 
    }
