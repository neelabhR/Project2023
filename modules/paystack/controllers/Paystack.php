...
class Paystack extends Hosting_Billing
{  
	...
	function __construct()
	{
        ...
        $this->config = get_settings('paystack');
    ...
    public function paystack_config ($values = null)
    {
        $config = array( 
            array(
                'label' => lang('paystack_secret_key'), 
                'id' => 'secret_key',
	...
	{		
        ...
        $client_cur = $inv->currency;       
        
        if($client_cur != 'NGN' && $client_cur != 'GHS') {
            $this->session->set_flashdata('response_status', 'error');
    ...
    function process ()
    {
        ...
        $this->session->set_userdata('paystack_ref', $ref);
      ...
      function verify() {
        ...
        $ref = explode('_', $this->session->userdata('paystack_ref'), -1);
        $inv = Invoice::view_by_ref($ref[0]);
        $input = array();
