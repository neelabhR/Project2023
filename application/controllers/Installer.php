...
class Installer extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        if ($this->_check_install() && $this->uri->segment(2) != 'done') {
            exit('Application already installed. Need help? http:
        }

        $this->load->helper(array('url', 'file', 'curl'));
        $this->update_url = 'http:
    }
    ...
    public function _valid_script()
    {
        $purchase_code = $this->input->post('set_envato_license');
        $update_url = $this->input->post('update_url');
        return curl_get_contents($update_url.'/verify/index.php?code='.$purchase_code);
    }
    

    public function _get_db($version = null)
    {
        if ($version) {
            $ch = curl_init();
